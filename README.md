...
**34 правила · 14 агентов · 25 скиллов · 5 типов проектов · 484 тестов · раскатка по HTTP, идемпотентная и обратимая**
...
Полезное, родившееся в конкретном проекте, выносится обратно в канон: бриф собирает синк или скилл `harvest-canon` (в конце любой задачи, где всплыл переиспользуемый прием). Это самодостаточный артефакт в `toolkit-log/upstream-pending/`, который можно скормить Claude в клоне toolkit, отревьюить и закоммитить. Доставка автоматизирована машинным inbox: `harvest-canon` кладет копию брифа в `~/.claude/canon-inbox/`, и сессия в клоне разбирает очередь по фразе "разбери inbox" - пути к брифам руками не переносятся. Проект ничего не знает, где на машине лежит клон - связь строго односторонняя, через явные артефакты. На следующем синке проект видит, что правило доехало, и чистит пометку. В fleet-контуре harvester [claude-control](https://github.com/dewil/claude-control) автоматизирует сбор таких кандидатов.
...
3. **[`tests/`](./tests/)** - 484 stdlib-тестов (100 из них - движок), включая crash-матрицу WAL и по регресс-тесту на каждую находку состязательного ревью. Инварианты движка быстрее всего считываются именно отсюда.
...
- `rules/` - канонические правила поведения Claude (34).
- `agents/` - канонические описания субагентов (14).
- `skills/` - канонические скиллы, папка на скилл (25).
