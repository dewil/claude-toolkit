...
В существующем проекте пользователь говорит "сделай синк с canon" (или "sync canon") - Claude читает `.claude/canon.yaml`, фетчит каждый `files[i]` с `raw_base`, сравнивает с локальным, спрашивает по каждому "обновить? (да/нет/diff)". Логика - в `migrations/sync-from-canon.prompt.md`.
