...
- **`prompt-reviewer`** (агент из `.claude/agents/`) - на **каждую** правку промт-файлов: `rules/`, `agents/`, `skills/`, `commands/`, `bootstrap/`, `migrations/`, `templates/`, `start.md`. Проверяет clarity / safety / идемпотентность / согласованность перекрестных ссылок. Запускается проактивно, без напоминания.
- **`codex-audit`**, режим **`adversarial`** - на **существенные** правки любого типа: новый актив, изменение логики промта или скрипта, правка движка (`scripts/`, `tests/`). Вторая модель ловит другие классы ошибок, чем автор правки; установка "найди, чем сломается" - а не "подтверди, что ок". Для промтов атаки формулируются в терминах исполняющего агента: на каком проекте, с каким содержимым файлов агент, добросовестно следующий тексту, сделает неверное действие.

Не триггерят аудит: опечатки, счетчики README, правки комментариев, типографика - мелочь пушится сразу.
