...
- Получи секцию `wiki` манифеста (`curl -fsSL <canon_base>/manifest.yaml`) - это целевой набор файлов специализации (единственный источник списка).
...
1. Из `.claude/canon.yaml -> files` удали записи старого типа. Список файлов типа = его секция в `manifest.yaml` (`curl -fsSL <canon_base>/manifest.yaml`; единственный источник). **Исключение:** не удаляй файлы, которые есть и в секции `wiki` - они расшарены и продолжают использоваться (на практике это `skills/telegram-snapshot/SKILL.md`, `scripts/telegram-snapshot.py`, `scripts/telegram-deltas.py`, общие с management). Заодно убери из `file_hashes` записи реально удаленных путей.
