...
- `format-dimensions-<проект>.md` - **конкретные** значения к каноническому `format-dimensions.md`: свои токены единиц (`sec`/`min`/`Gb`), число знаков, пороги переключения, имя функции форматирования. Сам прием раскатывается каноном всегда; проектным файлом задаются только эти значения - и только когда в проекте есть вывод величин с размерностью.
