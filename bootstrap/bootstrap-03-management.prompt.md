...
- скилл `mymeet-snapshot` - первая настройка/починка зеркала расшифровок встреч из mymeet.ai (API-ключ, whitelist-правила раскладки, review-файл, SSL через curl).
- скрипты в корне проекта:
  ...
  - `scripts/mymeet-snapshot.py` - pull MD-расшифровок проектных встреч из mymeet.ai по whitelist-правилам (без `-deltas`-пары: новые встречи = то, что прогон только что скачал).
