...
- `scripts/telegram-send-one.py` - драйвер single-chat-by-id: отправка в один чат по id, минуя конфиг (тот же гейт, сверка username, `--html`, `--file`). Переиспользует логику telegram-send.py через импорт.
