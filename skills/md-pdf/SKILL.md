...
- **Таблице обязательна строка-разделитель `|---|` второй строкой.** Без нее блок таблицей не считается и уходит абзацами - это защита от того, чтобы обычный текст с вертикальной чертой (`|x| - модуль числа`) молча не превратился в таблицу. Ширина строк выравнивается по шапке: лишние ячейки отбрасываются, недостающие добиваются пустыми, так что число колонок в шапке должно быть верным. Экранированная `\|` внутри ячейки ее не делит. Выравнивание (`:--:`) как разделитель принимается, но на верстку не влияет.
