...
- Документ со сложной версткой (таблицы, многоколоночность, вложенные списки) - конвертер их не умеет; либо дорабатывать конвертер, либо собирать HTML вручную и печатать тем же Chrome.
...
- **Текст в Chrome-PDF не грепается** (глифовые индексы, не юникод). Верифицировать содержимое по сгенерированному HTML (функция `md_to_html`), а не по PDF; глазами PDF смотрит пользователь.
