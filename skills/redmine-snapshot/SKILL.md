...
4. **Снапшот - технический артефакт: его место вне синкаемой папки.** `tasks_root` принимает абсолютный путь - им снимок и уводится из проекта; скрипт предупреждает на каждом прогоне, если путь ведет внутрь. Исключение из синка после создания файла не спасает - только защищает следующий (`rules/docs-maintenance.md`, раздел "Технические артефакты в синкаемой папке").
